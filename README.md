...
> Built an end-to-end churn analytics pipeline (XGBoost + SHAP + VADER sentiment
> analysis on support-ticket text) on a 2,500-customer SaaS dataset, achieving
> 0.81 ROC-AUC (0.848 ± 0.024 cross-validated). Identified usage depth as the
> dominant churn driver (81.5% churn among the lowest-usage third of customers)
> and validated the finding with SHAP interpretability. Translated results into
> a RICE-prioritized product roadmap, designed a statistically powered A/B test
> (via `statsmodels`) for the top intervention, and projected $301K+ in annual
> protected revenue against a 4-person-week engineering cost.
...
.
├── README.md                      <- this file
├── Self_project_complete.ipynb    <- full analysis notebook (run in Colab)
├── top_at_risk_customers.csv      <- risk-scored customer list (model output)
├── data/
│   ├── train.csv
│   └── test_.csv
└── visuals/
...
1. Open `Self_project_complete.ipynb` in Google Colab
2. Run the setup cell to mount Google Drive and install dependencies
3. Ensure `train.csv` and `test_.csv` are in your Drive's root (`MyDrive/`)
4. Run all cells — outputs (plots, CSVs, model file) save back to your Drive
